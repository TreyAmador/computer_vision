...
def blend(A,B):
    gpA = gen_gaussian(A)
    gpB = gen_gaussian(B)
    lpA = gen_laplacian(gpA)
    lpB = gen_laplacian(gpB)
    LS,rows,cols = add_images(lpA,lpB)
    LB = reconstruct(LS)
...
def driver():
    ...
    ls,real = blend(A,B)
    cv2.imwrite('img/pyr_blending.jpg',ls)
